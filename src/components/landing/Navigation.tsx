...
export const Navigation: React.FC<NavigationProps> = ({ currentSection = 'home' }) => {
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 px-6 pointer-events-none">
      ...
      {/* Menu Bubble (Center - absolutely positioned) */}
      <motion.div
        initial={{ y: -50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        className="pointer-events-auto absolute top-0 left-1/2 -translate-x-1/2 bg-[#0F1115]/80 backdrop-blur-xl border border-white/10 rounded-full px-2 py-2 flex items-center shadow-lg"
      >
